...
def _extract_pairs(rates: Iterable[Dict[str, Any]]) -> List[Tuple[str, Decimal]]:
    """Yield (carrierCode, price) with normalized code and Decimal price, filtering malformed rows."""
    ...
    for r in rates or ():
        ...
        if not code or price is None:
            continue
        # Ensure Decimal for consistent comparisons/pretty-printing
        pairs.append((_norm(code), Decimal(str(price))))
    return pairs

def _first_non_usps(pairs: List[Tuple[str, Decimal]]) -> Optional[Tuple[str, Decimal]]:
    return next((p for p in pairs if not _is_usps(p[0])), None)
...
class ShipHelper(BaseEndpoint):
    ...
    def __init__(self):
        ...
        from ABConnect.api.endpoints.jobs.freightproviders import JobFreightProvidersEndpoint
    ...
    def _get_choices(self, job_display_id: str, ensure_provider: Optional[str] = None) -> Tuple[str, Dict[str, Decimal]]:
            """
            ...
            Returns an ordered dict (insertion order preserved) mapping carrierCode -> price.
            """
            ...
            if not pairs:
                logger.info("No rate quotes for job %s; returning empty choice set.", job_display_id)
                return {}

            # Always include the first (lowest) rate.
            first_code, first_price = pairs[0]
            choices: Dict[str, Decimal] = {first_code: first_price}
            ...
            if _is_usps(first_code):
                non_usps = _first_non_usps(pairs)
                if non_usps:
                    code2, price2 = non_usps
                    choices[code2] = price2

            # If ensure_provider is requested but not present yet, add its rate if available.
            if ensure and ensure not in choices:
                ensured = next((p for p in pairs if p[0] == ensure), None)
                if ensured:
                    code3, price3 = ensured
                    choices[code3] = price3
